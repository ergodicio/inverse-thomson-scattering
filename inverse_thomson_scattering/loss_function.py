...
class SpectrumCalculator(hk.Module):
    def __init__(self, cfg, sas, dummy_batch):
        ...
        if (
            ...
            self.vmap_forward_pass = hk.vmap(self.forward_pass)
            self.vmap_postprocess_thry = hk.vmap(self.postprocess_thry)
...
class LossFunction(hk.Module):
    def __init__(self, cfg, sas, dummy_batch):
        super().__init__()
        self.cfg = cfg
        self.calculate_spectra = SpectrumCalculator(cfg, sas, dummy_batch)
        self.param_generator = TSParameterGenerator(cfg)
    ...
    def prepare_input_from_batch(self, batch):
        return {
            "data": self.get_normed_batch(batch),
    ...
    def get_params(self, batch):
        return self.param_generator(self.prepare_input_from_batch(batch))

    def __get_active_params__(self, batch):
        return self.param_generator.get_active_params(self.prepare_input_from_batch(batch))

    def __call__(self, batch):
        params = self.get_params(batch)
        ThryE, ThryI, lamAxisE, lamAxisI = self.calculate_spectra(params, batch)
        i_error = 0.0
        e_error = 0.0
        ...
        if self.cfg["other"]["extraoptions"]["fit_IAW"]:
            ...
            _error_ = jnp.square(i_data - ThryI) / jnp.square(self.i_norm)
            _error_ = jnp.where(
        ...
        if self.cfg["other"]["extraoptions"]["fit_EPWb"]:
            _error_ = jnp.square(e_data - ThryE) / jnp.square(self.e_norm)
            _error_ = jnp.where(
