...
#persistent DFNOld;
    ...
    [DFName,params]=GenTableName(*args,nout=2)
    ...
    if 'xs' in locals():
        ...
        if len(curDist) == 1:
            ...
            [X1, X2] = np.meshgrid(xs[ci],curDist[1])
            interpedSection = sp.interpn((params['x'],params['m']),IT,(X1,X2))
        elif len(curDist) == 3:
            [X1, X2, X3] = np.meshgrid(xs[ci],curDist[1],curDist[2])
            interpedSection = sp.interpn((params['x'],params['m'],params['Z']),IT,(X1,X2,X3))
        elif len(curDist) == 4:
            if len(curDist[2])==1: #this currently allows the angle for Spitzer to be specificed as a single angle or a range but there might be a better solution
                [X1, X2, X3, X4] = np.meshgrid(xs[ci],curDist[1],curDist[2],curDist[3])
                interpedSection = sp.interpn((params['x'], params['m'], params['theta'], params['delT']), IT, (X1,X2,X3,X4))
...
Dramatically stripped down for python rewrite: 8/19/22 A.L.Milder'''
    ...
    for key in inputs:
        params[inputs[key][0]]=np.arange(inputs[key][2][0],inputs[key][2][1]+10**inputs[key][1],10**inputs[key][1])
