...
class DistFunc:
    def __init__(self, config):
        ...
        if "f1_direction" in config["parameters"]["fe"].keys():
            self.f1_direction = config["parameters"]["fe"]["f1_direction"]/jnp.sqrt(jnp.sum([ele**2 for ele in config["parameters"]["fe"]["f1_direction"]])) 
        #add static variable for all the possible fields of the dictionary instead of using a dictionary
    def __call__(self, fedict):
    
        if self.fe_name == "DLM":
            if self.dim == 1:
                v,fe = dist_functional_forms.DLM_1D(fedict["m"], self.velcity_res)
            elif self.dim == 2:
                v,fe = dist_functional_forms.DLM_2D(fedict["m"], self.velcity_res)

        elif self.fe_name == "Spitzer":
            if self.dim == 2:
                if len(self.f1_direction) == 2:
                    v,fe = dist_functional_forms.Spitzer_2V(self.dt, self.f1_direction, self.velcity_res)
                elif len(self.f1_direction) == 3:
                    v,fe = dist_functional_forms.Spitzer_3V(self.dt, self.f1_direction, self.velcity_res)
            else:
            ...
            if self.dim == 2:
                if len(self.f1_direction) == 2:
                    v,fe = dist_functional_forms.MoraYahi_2V(self.dt, self.f1_direction, self.velcity_res)
                elif len(self.f1_direction) == 3:
                    v,fe = dist_functional_forms.MoriYahi_3V(self.dt, self.f1_direction, self.velcity_res)
            else:
